            ...
            content += `<tr>
                            ...
                            <td>
                                <div class="btn-group gap-1" role="group">
                                    ...
                                    <button type="button" class="btn btn-outline-danger" onclick="confirmDelete(${item.id})" title="Eliminar">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" 
...
const loadStorage = async id => {
    ...
    let userSelect = document.getElementById('u_userList');
    let content = '';
    ...
    let availableUsers = userList.filter(user => 
        !usersWithStorage.includes(user.id) || user.id === storage.user.id
    );
    ...
    userSelect.value = storage.user.id;
}
...
const search = async () => {
    ...
    const storageInput = document.getElementById('searchStorage');

    // Obtener el nombre de la categoría desde el input
    let storageName = storageInput.value.trim().toLowerCase();

    // Filtrar categorías que contengan el texto buscado
    let foundStorages = storageList.filter(item => item.name.toLowerCase().includes(storageName));
    ...
    if (foundStorages.length > 0) {
        ...
        foundStorages.forEach((item, index) => {
            const categoryClass = getCategoryBadgeClass(item.category.id);
            const lastname = item.user.lastname ? item.user.lastname : '';
...
const manageItems = async (id) => {
    ...
    confirmButton1.addEventListener('click', async () => {
        ...
        selectedInsertArticles = [];
        console.log('selectedInsertArticles después de confirmar', selectedInsertArticles);
    });
    ...
    confirmButton2.addEventListener('click', async () => {
        ...
        selectedOutArticles = [];
        console.log('selectedOutArticles después de confirmar', selectedOutArticles);
    });
...
const updateStorage = async () => {
    ...
    let updated = {
        ...
        user: {
            id: document.getElementById('u_userList').value
        },
        category: {
...
const deleteStorage = async (storageId) => {
    ...
    if (articleIds.length > 0) {
            showError("No puedes eliminar almacén", "Este almacén tiene artículos dentro");
    } else {
        await fetch(`${URL}/api/storage`, {
            method: 'DELETE',
            headers: {
...
const filterByCategory = async () => {
    ...
    // Filtrar los almacenes que pertenezcan a la categoría seleccionada
    const filteredStorages = storageList.filter(storage => 
        storage.category && storage.category.id == selectedCategoryId
    );
    ...
    if (filteredStorages.length > 0) {
        // Llenar la tabla con los almacenes filtrados
        filteredStorages.forEach((storage, index) => {
            const categoryClass = getCategoryBadgeClass(storage.category.id);
            const lastname = storage.user.lastname ? storage.user.lastname : '';

            content += `<tr>
                            <th scope="row">${index + 1}</th>
                            <td>${storage.name}</td>
                            <td>${storage.user.name} ${storage.user.surname} ${lastname}</td>
                            <td><h5><span class="badge ${categoryClass} user-select-none shadow-sm">${storage.category.name}</span></h5></td>
                            <td>
                                <div class="btn-group gap-1" role="group">
                                    <button type="button" class="btn btn-outline-info" data-bs-toggle="modal" data-bs-target="#readStorage" onclick="readStorage(${storage.id})" title="Ver más">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                            fill="currentColor" class="bi bi-eye" viewBox="0 0 16 16">
                                            <path d="M16 8s-3-5.5-8-5.5S0 8 0 8s3 5.5 8 5.5S16 8 16 8M1.173 8a13 13 0 0 1 1.66-2.043C4.12 4.668 5.88 3.5 8 3.5s3.879 1.168 5.168 2.457A13 13 0 0 1 14.828 8q-.086.13-.195.288c-.335.48-.83 1.12-1.465 1.755C11.879 11.332 10.119 12.5 8 12.5s-3.879-1.168-5.168-2.457A13 13 0 0 1 1.172 8z" />
                                            <path d="M8 5.5a2.5 2.5 0 1 0 0 5 2.5 2.5 0 0 0 0-5M4.5 8a3.5 3.5 0 1 1 7 0 3.5 3.5 0 0 1-7 0" />
                                    ...
                                    <button type="button" class="btn btn-outline-primary" data-bs-toggle="modal" data-bs-target="#updateStorage" onclick="loadStorage(${storage.id})" title="Editar">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                            fill="currentColor" class="bi bi-pencil-square" viewBox="0 0 16 16">
                                            <path d="M15.502 1.94a.5.5 0 0 1 0 .706L14.459 3.69l-2-2L13.502.646a.5.5 0 0 1 .707 0l1.293 1.293zm-1.75 2.456-2-2L4.939 9.21a.5.5 0 0 0-.121.196l-.805 2.414a.25.25 0 0 0 .316.316l2.414-.805a.5.5 0 0 0 .196-.12l6.813-6.814z" />
                                            <path fill-rule="evenodd" d="M1 13.5A1.5 1.5 0 0 0 2.5 15h11a1.5 1.5 0 0 0 1.5-1.5v-6a.5.5 0 0 0-1 0v6a.5.5 0 0 1-.5.5h-11a.5.5 0 0 1-.5-.5v-11a.5.5 0 0 1 .5-.5H9a.5.5 0 0 0 0-1H2.5A1.5 1.5 0 0 0 1 2.5z" />
                                    ...
                                    <button type="button" class="btn btn-outline-warning" data-bs-toggle="modal" data-bs-target="#itemControl" onclick="manageItems(${storage.id})" title="Gestionar artículos en almacén">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" class="bi bi-boxes" viewBox="0 0 16 16">
                                            <path d="M7.752.066a.5.5 0 0 1 .496 0l3.75 2.143a.5.5 0 0 1 .252.434v3.995l3.498 2A.5.5 0 0 1 16 9.07v4.286a.5.5 0 0 1-.252.434l-3.75 2.143a.5.5 0 0 1-.496 0l-3.502-2-3.502 2.001a.5.5 0 0 1-.496 0l-3.75-2.143A.5.5 0 0 1 0 13.357V9.071a.5.5 0 0 1 .252-.434L3.75 6.638V2.643a.5.5 0 0 1 .252-.434zM4.25 7.504 1.508 9.071l2.742 1.567 2.742-1.567zM7.5 9.933l-2.75 1.571v3.134l2.75-1.571zm1 3.134 2.75 1.571v-3.134L8.5 9.933zm.508-3.996 2.742 1.567 2.742-1.567-2.742-1.567zm2.242-2.433V3.504L8.5 5.076V8.21zM7.5 8.21V5.076L4.75 3.504v3.134zM5.258 2.643 8 4.21l2.742-1.567L8 1.076zM15 9.933l-2.75 1.571v3.134L15 13.067zM3.75 14.638v-3.134L1 9.933v3.134z"/>
                                        </svg>
                                    </button>
                                    <button type="button" class="btn btn-outline-danger" onclick="confirmDelete(${storage.id})" title="Eliminar">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" 
                                            fill="currentColor" class="bi bi-trash" viewBox="0 0 16 16">
                                            <path d="M5.5 5.5A.5.5 0 0 1 6 6v6a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5m2.5 0a.5.5 0 0 1 .5.5v6a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5m3 .5a.5.5 0 0 0-1 0v6a.5.5 0 0 0 1 0z"/>
                                            <path d="M14.5 3a1 1 0 0 1-1 1H13v9a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V4h-.5a1 1 0 0 1-1-1V2a1 1 0 0 1 1-1H6a1 1 0 0 1 1-1h2a1 1 0 0 1 1 1h3.5a1 1 0 0 1 1 1zM4.118 4 4 4.059V13a1 1 0 0 0 1 1h6a1 1 0 0 0 1-1V4.059L11.882 4zM2.5 3h11V2h-11z"/>
        ...
        content = `<tr>
                <td colspan="5" class="text-center" style="height: 200px;">
                    No hay registros que coincidan con la categoría seleccionada
                </td>
            </tr>`;
    }
